...
RSpec.describe AdvertiserAuthorizeApiRequest do
  ...
  describe '#call' do
    ...
    context 'when valid request' do
      it "returns advertiser object" do
        result = request_obj.call
        expect(result[:advertiser]).to eq(advertiser)
      end
    end
    ...
    context 'when invalid request' do
      ...
      context 'when invalid token' do
        subject(:invalid_request_obj) do
          # custom helper method 'advertiser_token_generator'
          described_class.new('Authorization' => advertiser_token_generator(5))
        end
      ...
      context 'when token is expired' do
        let(:header) { { 'Authorization' => expired_advertiser_token_generator(advertiser.id) } }
        subject(:request_obj) { described_class.new(header) }
