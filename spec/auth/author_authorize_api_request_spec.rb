...
RSpec.describe AuthorAuthorizeApiRequest do
  # Create test user
  let(:author) { create(:author) }
  let(:header) { { 'Authorization' => author_token_generator(user.id) } }
    ...
    context 'when invalid request' do
      ...
      context 'when invalid token' do
        subject(:invalid_request_obj) do
          # custom helper method 'author_token_generator'
          described_class.new('Authorization' => author_token_generator(5))
        end
      ...
      context 'when token is expired' do
        let(:header) { { 'Authorization' => expired_author_token_generator(author.id) } }
        subject(:request_obj) { described_class.new(header) }
